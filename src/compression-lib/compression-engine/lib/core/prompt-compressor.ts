...
function softmaxProbabilityForClass(
  ...
  for (let i = 0; i < numClasses; i++) {
    const value = Number(logits[offset + i]);
    if (value > maxLogit) {
      maxLogit = value;
    }
  }
  ...
  for (let i = 0; i < numClasses; i++) {
    const value = Math.exp(Number(logits[offset + i]) - maxLogit);
    denominator += value;
    if (i === classIndex) {
      target = value;
    }
 ...
 */
export class PromptCompressorCore {
  ...
  constructor(
    ...
    for (let i = 0; i < this.engineConfig.max_force_token; i++) {
  ...
  private chunkContext(
    ...
    const maxLenTokens = this.engineConfig.max_seq_length - 2;
  ...
  private async compressContexts(
    ...
    const chunked_contexts = chunk(
      contexts,
      this.engineConfig.max_batch_size
    );
    ...
    for (const context of chunked_contexts) {
      ...
      for (let j = 0; j < batch_size; j++) {
        ...
        const active_probs = chunk_mask_number_array.flatMap((mask, i) => {
          if (mask <= 0) {
            return [];
          }

          const offset = (j * seq_len + i) * num_classes;
          return [softmaxProbabilityForClass(logits, offset, num_classes, 1)];
        });

        const active_ids = chunk_ids.data
          .filter((_: bigint, i: number) => chunk_mask_number_array[i] > 0)
